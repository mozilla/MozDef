...
GeoModel v0.1 is a standalone program that can be run to:

  1. Process authentication-related events.
  2. Update user locality information.
  3. Emit events when some configured conditions are met.

### Data Stores

GeoModel interacts with MozDef to both query for as well as store new events.
...
The second component handles the creation of events and communicating of those
events to MozDef.

This component has the following responsibilities:

  1. Inspect localities produced by the Analysis Engine to produce events.
  2. Store events in MozDef's ElasticSearch instance.
...
if and only if both:
  ...
  locality from the one they were last active from.
...
For example, the following data structure describes a JSON object with some
simple and nested data types.
...
Using the information above, GeoModel can determine:
  ...
  * What index to read events from and write events to.
...
these objects are expected to be an [ElasticSearch query](
