...
class AlertAuth0BruteforceUser(AlertTask):
    def main(self):
        self.parse_config('auth0_bruteforce_user.conf', ['threshold_count', 'search_depth_min'])
    ...
    def onAggregation(self, aggreg):
        ...
        for event in aggreg['allevents']:
            client_list.add(event['_source']['details']['sourceipaddress'])

        summary = 'Auth0 Bruteforce Attack in Progress against user ({0}) from the following source ip(s): {1}'.format(
            aggreg['value'],
