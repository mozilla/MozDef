...
def index():
    ...
    if "ipaddress" in requestDict and isIPv4(requestDict["ipaddress"]):
        ...
        dresponse = requests.get(
            "{0}{1}?json".format(url, requestDict["ipaddress"]), headers=headers
        )
...
def index():
    ...
    schedulers_db = mongoclient.meteor["alertschedules"].with_options(
...
def sync_alert_schedules():
    ...
    for result in results:
        if result["name"] in alert_schedules:
            ...
            logger.debug(
                "Inserting schedule for {0} into mongodb".format(result["name"])
            )
...
def createIncident():
    ...
    if not request.body:
        response.status = 500
        response.body = json.dumps(dict(status="failed", error="No data provided"))
    ...
    try:
        ...
        response.status = 500
        response.body = json.dumps(dict(status="failed", error="Invalid JSON"))
    ...
    try:
        ...
        response.body = json.dumps(
            dict(
                status="failed",
                error="Missing required keys" "(summary, phase, creator)",
            )
        )
        return response
    ...
    if not valid_phase:
        response.status = 500
        response.body = json.dumps(
            dict(status="failed", error="Invalid incident phase")
        )
        return response
    ...
    if not re.match(EMAIL_REGEX, incident["creator"]):
        response.status = 500
        response.body = json.dumps(dict(status="failed", error="Invalid creator email"))
        return response
    ...
    if False in dates:
        ...
        response.body = json.dumps(
            dict(
                status="failed",
                error="Wrong format of date. Please " "use yyyy-mm-dd hh:mm am/pm",
            )
        )
        return response
    ...
    if incident["tags"] and type(incident["tags"]) is not list:
        response.status = 500
        response.body = json.dumps(
            dict(status="failed", error="tags field must be a list")
        )
        return response
    ...
    if incident["references"] and type(incident["references"]) is not list:
        response.status = 500
        response.body = json.dumps(
            dict(status="failed", error="references field must be a list")
        )
        return response
    ...
    try:
        ...
        response.status = 500
        response.body = json.dumps(dict(status="failed", error=err))
        return response
    ...
    response.body = json.dumps(
        dict(
            status="success",
            message="Incident: <{}> added.".format(incident["summary"]),
        )
    )
    return response
            ...
            if not module:
                ...
                if "message" in dir(module):
                    ...
                    if isinstance(mreg, list):
                        logger.info(
                            "[*] plugin {0} registered to receive messages from /{1}".format(
                                mfile, mreg
                            )
                        )
                        pluginList.append(
                            (mfile, mname, mdescription, mreg, mpriority, mclass)
                        )
...
def kibanaDashboards():
    ...
    try:
        ...
        for dashboard in results["hits"]:
            ...
            resultsList.append(
                {"name": dashboard["_source"]["dashboard"]["title"], "id": dashboard_id}
            )
...
def getWatchlist():
    ...
    try:
        ...
        expired = watchlistentries.find(
            {"dateExpiring": {"$lte": datetime.utcnow() - timedelta(hours=1)}}
        )
        for entry in expired:
            logger.debug("Deleting entry {0} from watchlist /n".format(entry))

        # delete any that expired
        watchlistentries.delete_many(
            {"dateExpiring": {"$lte": datetime.utcnow() - timedelta(hours=1)}}
        )
    ...
    try:
        ...
        iveris = incidents.aggregate(
            [
                ...
                {"$unwind": "$tags"},
                {"$match": {"tags": {"$regex": ""}}},
                {"$project": {"dateOpened": 1, "tags": 1, "phase": 1, "_id": 0}},
            ]
        )
        if iveris:
...
def initConfig():
    ...
    default_user_agent = (
        "Mozilla/5.0 (X11; Linux x86_64; rv:10.0) Gecko/20100101 Firefox/58.0"
    )
