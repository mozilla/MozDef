...
def esSearch(es, begindateUTC=None, enddateUTC=None):
    ...
    try:
        ...
        qAdd = pyes.QueryFilter(pyes.MatchQuery("changetype","add","phrase"))

        q = pyes.ConstantScoreQuery(pyes.MatchAllQuery())
        q = pyes.FilteredQuery(q,pyes.BoolFilter(must=[qDate, qType, qAdd],must_not=[qAlerted]))
        ...
        for r in rawresults['hits']['hits']:
            alert = dict(
                ...
                severity='INFO',
                summary='{0} added {1}'.format(r['_source']['details']['actor'],r['_source']['details']['dn']),
                category='ldap',
                tags=['ldap'],
                eventsource=[],
                events=[])
