...
def esHostScannerEvents():
    ...
    qType = pyes.TermFilter('_type', 'cef')
    qEvents = pyes.QueryFilter(pyes.MatchQuery("signatureid","sensitivefiles","phrase"))
        ...
        if len(indicatorCounts) > 0:
            for i in indicatorCounts:
                if i['count'] > threshold:
                    alert = dict(utctimestamp=toUTC(datetime.now()).isoformat(), severity='NOTICE', summary='', category='hostscanner', tags=['hostscanner'], eventsource=[], events=[])
                    for e in i['events']:
                        alert['events'].append(
                    ...
                    for e in i['events'][0:3]:
                        if 'path' in e['_source']['details'].keys():
                            alert['summary'] += '{0} '.format(e['_source']['details']['path'])
                    for e in i['events']:
